...
def setupCamera():
    ...
    if fp_mode:
        ...
        cam_x = player_x - math.cos(rad) * 30
        cam_y = player_y - math.sin(rad) * 30
        cam_z = 75

        look_x = player_x + math.cos(rad) * 200
        look_y = player_y + math.sin(rad) * 200
        look_z = 60
...
def idle():
    ...
    for b in bullets[:]:
        b['x'] += math.cos(math.radians(b['angle'])) * 10
        b['y'] += math.sin(math.radians(b['angle'])) * 10
